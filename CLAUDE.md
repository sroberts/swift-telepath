...
`aha://` resolution is implemented (spec §3.9, M7). Mirror pools are specified but not built (M8): a name that resolves to a pool is refused, never silently connected to one member.
