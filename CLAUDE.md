...
`aha://` resolution and AHA mirror pools are implemented (spec §3.9, M7 and M8). A pool is `AHAPool`, never a `Proxy` — a pool has no single session, so `sessionIden` and `state` would be lies — and the two refuse each other: `Proxy.open` on a pool throws `ahaIsAPool`, `AHAPool.open` on a single service throws `ahaIsNotAPool`.
...
- **A pool resets before every topology attempt, not after they all fail.** `followTopology` calls `shutDownMembers()` once a registry is reachable and before its replay begins, mirroring Python's `_toposync`. Resetting only after every registry failed passes a single-registry test and is wrong with two: a stream *error* falls through to the next registry, whose replay would merge onto membership describing a moment that has passed. Members are also evicted on their own `Proxy.state` reporting `disconnected`, because `Proxy` never re-handshakes and AHA cannot see a partition between this client and one member.
- **`aha://` resolution has four rules that inference gets wrong**, all read from `synapse/telepath.py` and `synapse/lib/aha.py`. Resolution runs over a *set* of registries and falling through all of them is a failure, not an empty success. `filters` goes only to a registry running >= 2.95.0, and a `?mirror=` request an older one cannot serve must fail rather than quietly return the leader. Merge precedence is local `path`, local `user` **only if the caller gave one**, upstream everything else — and AHA fills `user` in from the *requesting* user, so reversing that rule looks correct in every single-user test. A pool is signalled by a `services` key on the *outer* record, not inside the nested `svcinfo`.
- **`main` is protected and admins are not exempt.** All seven CI checks are required, `strict` is on, and force-pushes and deletions are blocked. Every change goes through a branch and a pull request, including yours — a direct push to `main` is rejected by the server. That is why CI wall-clock time is worth defending: it is now in the path of every merge. To lift it for an emergency, `gh api -X DELETE repos/sroberts/swift-telepath/branches/main/protection/enforce_admins`, and put it back afterwards.
- **The `Synapse` facade models Synapse 2.x and refuses other majors.** `Cortex.init` throws `CortexError.unsupportedSynapseVersion` on any reported major but 2, because 3.0's node payload decodes *without error* and yields wrong values — `iden` is absent (it is `nid` now) and every prop is a `(value, metadata)` tuple that `Node` copies through as if it were the value. A server reporting no version is allowed through; absence is not evidence. `Telepath` is deliberately not gated — it is version agnostic and verified against 3.0.0. Measurements in `docs/synapse-3.0.md`, decision in `spec.md` §8.
- **Every Linux CI job runs in `swift:6.2-noble`, and that is load-bearing twice over.** A toolchain installed per-run is not byte-identical between runs, so the dependency cache restores and SwiftPM recompiles the whole graph regardless — the unit job sat at 449s with a 258 MB cache hit until it moved into the image. The pinned image is also what lets every Linux job share one cache entry. The second consequence: container steps run under `sh -e {0}`, not bash, so **no bashisms in any Linux job** — `set -o pipefail` has aborted a step before a single test ran.
- **CI caches compiled dependencies, never our own code's freshness.** The cache key covers `Package.resolved` and the toolchain only, so SwiftPM keeps the dependency graph compiled while everything in `Sources/` and `Tests/` still rebuilds. The **Documentation job is deliberately uncached** — it is the warning gate, and an incremental build does not re-emit warnings for files it did not recompile. Do not add a cache step to it to save three minutes.
